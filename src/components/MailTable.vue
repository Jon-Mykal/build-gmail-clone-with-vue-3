...
<h1>VMail Inbox</h1>
    <table class="mail-table">
      <tbody>
        <tr v-for="email in unarchivedEmails" :key="email.id" :class="[email.read ? 'read': '', 'clickable']" @click="email.read = !email.read">
          <td>
            <input type="checkbox" />
          </td>
          <td>{{ email.from }}</td>
          ...
          <td><button @click="email.archived = !email.archived">Archive</button></td>
        </tr>
      </tbody>
    </table>
</template>
